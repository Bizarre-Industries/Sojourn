...
3. **Determine council trigger.** Per CLAUDE.md "Council (the only
   gate)" list:
   - new ADR creation
...
5. **Write the tests.** Under `SojournTests/Services/<Name>Tests.swift`
   ...
   subprocess flow runs once locally as smoke (per CLAUDE.md
   "smoke-run before claiming done").

6. **Smoke check.** Per CLAUDE.md verification rule: any code that
...
11. **Commit.** Stage explicit files (never `git add -A`). Use
    ...
    - <bullet>
    ```
    `git commit -s`. Never `--no-verify`. No `Co-Authored-By: Claude`.
...
- **Don't skip the smoke check** because tests pass. CLAUDE.md
