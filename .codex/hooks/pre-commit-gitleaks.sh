...
# files. Per AGENTS.md invariant 9: "gitleaks runs before every
...
# Find repo root (the hook runs from CODEX_PROJECT_DIR when available, but the
# user's commit may be from a subdir).
repo_root="${CODEX_PROJECT_DIR:-${CLAUDE_PROJECT_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}}"
