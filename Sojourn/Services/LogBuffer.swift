...
internal struct LogLine: Sendable, Hashable {
  internal let stream: StreamTag
  internal let text: String
  internal let timestamp: Date

  internal init(stream: StreamTag, text: String, timestamp: Date = Date()) {
    self.stream = stream
    self.text = text
...
internal actor LogBuffer {
  internal let id: LogBufferID
  ...
  internal init(id: LogBufferID = LogBufferID(), capacity: Int = 10_000) {
    self.id = id
  ...
  internal func append(_ line: LogLine) {
  ...
  internal func feed(_ chunk: StreamChunk) {
