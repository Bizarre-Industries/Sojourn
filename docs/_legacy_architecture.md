...
> **Diagram caveat.** The diagram above reflects the **v0.1 layering**. The target v1.x layout (per [AUDIT.md §3](AUDIT.md#3-architectural-gaps)) adds `Models/`, `Policy/`, `Plugins/`, `Diagnostics/`, `Secrets/` modules. Those land in [IMPLEMENTATION_PLAN.md](IMPLEMENTATION_PLAN.md) phases 10–14.
...
- **The six conflict shapes** (text edit, packages.toml diverged, chezmoi template, plist, rename vs edit, delete vs edit) are enumerated in [CONFLICTS.md](CONFLICTS.md). The v0.1 design PDF (page 16) renders shapes 1 and 4; the doc is the canonical list.
...
**Discover pane (forward reference).** The v0.1 design PDF (page 36) introduces a "Discover" pane backed by a `cfprefsd` watcher that records preference changes as the user makes them. This subsystem is **not specced in this section** — it depends on a maintainer decision (see [AUDIT.md §8 Q4](AUDIT.md#8-open-questions--decisions-deferred-to-maintainer)) about scope: opt-in-per-domain recording, global recording with redaction, or something else. Full spec lands in `docs/explain/discover-pane.md` once the decision is made.
...
- Cork's license is GPL-3 with paid binaries via Paddle (verified per [AUDIT.md §1.2](AUDIT.md#1-doc-level-inconsistencies)). No Commons Clause; the source is open under GPL-3.
